...
fn base_cfg(tmp: &tempfile::TempDir) -> Config {
    ...
    cfg.limits.insert("openai".into(), mdaicli::config::LimitsConfig{ requests_per_minute: 10, tokens_per_minute: 100000, max_retries: Some(0), backoff_base_ms: Some(1), backoff_max_ms: Some(2) });
    cfg
}
...
fn openai_nonstream_success() {
    ...
    // Build query
    let q = mdaicli::opts::Query { model: Some("gpt-4".into()), system: None, user: Some("hi".into()), messages_file: None, input_files: vec![], input_role: "user".into(), temperature: None, top_p: None, max_tokens: None, stream: false, format: "json".into(), timeout: Some(10), tools_file: None, output: None };
...
fn openai_429_rate_limit_error() {
    ...
    if let Some(l) = cfg.limits.get_mut("openai") { l.max_retries = Some(0); }

    let q = mdaicli::opts::Query { model: Some("gpt-4".into()), system: None, user: Some("hi".into()), messages_file: None, input_files: vec![], input_role: "user".into(), temperature: None, top_p: None, max_tokens: None, stream: false, format: "json".into(), timeout: Some(5), tools_file: None, output: None };

    let res = mdaicli::providers::run_query(&cfg, "default", &q);
    ep.assert();
    assert!(res.is_err());
    let err = res.err().unwrap();
    assert!(matches!(err.kind, mdaicli::errors::ErrorKind::RateLimit | mdaicli::errors::ErrorKind::Provider));
}
