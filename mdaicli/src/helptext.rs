...
- Messages file + streaming JSONL:
  mdaicli query -p openai -m gpt-4 \
    --messages-file chat.json --stream -f json
...
3) list — List providers, models, credentials, cache
Usage:
...
4) remove — Remove credential or cache
Usage:
