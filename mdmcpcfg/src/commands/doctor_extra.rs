...
fn parse_wsl_target(cfg: &ClaudeDesktopConfig) -> Option<(Option<String>, String, String)> {
    ...
    while i < args.len() {
        if let Some(s) = args[i].as_str() {
            ...
            if !policy.is_empty() { return Some((distro, server, policy)); }
            return None;
        }
        i += 1;
    }
...
pub async fn check_wsl_side(
    ...
    if let Some((distro, server, policy)) = parse_wsl_target(&cfg) {
        ...
        if out.status.success() {
            ...
            issues.push(format!("Linux server missing or not executable: {}", server));
            println!("   ERROR: Linux server missing or not executable: {}", server);
        }
        // test -f policy
